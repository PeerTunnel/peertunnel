...
class TLSServer {
  ...
  handler (socket) {
    ...
    const next = async () => {
      ...
      if (zone.user) {
        // open a tunnel
        this.main.tunnels.requestTunnel(zone.user, { voidOnError: true, remote: { address: socket.remoteAddress, port: socket.remotePort, family: socket.remoteFamily } }, (err, remote) => { // the complete magic of this thing
